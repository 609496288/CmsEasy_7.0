...
{if $d['level']==0}<a onclick="child(this)" title="点击展开/收起" class="child"><i class="glyphicon glyphicon-folder-open"></i></a>{/if}
<?php } ?>
</td>
...
<a href="<?php echo url("archive/list/catid/$d[$primary_key]",false);?>" target="_blank" title="查看"><i class="glyphicon glyphicon-eye-open"></i></a>
...
<a href="<?php echo modify("/act/list/table/archive/catid/$d[$primary_key]");?>" title="管理"><i class="glyphicon glyphicon-list-alt"></i></a>
</td>
</tr>
{/loop}
</tbody>
...
<input class="btn btn-primary" type="button" value=" 移动到 " name="delete" onclick="if(getSelect(this.form) && confirm('确实要移动ID为('+getSelect(this.form)+')的栏目吗?')){this.form.action='<?php echo modify('act/batch',true);?>'; this.form.batch.value='move'; this.form.submit();}"/>

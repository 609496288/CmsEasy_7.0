...
<div class="col-xs-12 col-sm-6 col-md-6 col-lg-6">
     <form name="searchform" id="searchform"  action="<?php echo uri();?>" method="post">
			...
			专题&nbsp;
            <?php echo form::select('search_spid',get('search_spid')?get('search_spid'):0,special::option()); ?><div class="linebreak"><div class="blank10"></div></div>
            标题
            <input type="text" class="form-control" name="search_title" id="search_title" value="" />

            <input type="submit" value="搜索" name="submit"  onclick="this.form.action='{url::modify('table/'.get('table').'/type/search')}'" class="btn btn-info search-btn" />
        </form>
		</div>
<div class="linebreak"><div class="blank10"></div></div>
...
{if chkpower('archive_check')}<input type="button" value="审核内容" onclick="javascript:window.location.href='{url::create('table/list/table/archive/needcheck/1')}'"  class="btn btn-secondary" />&nbsp;{/if}
<input type="button" value="回收站" onclick="javascript:window.location.href='{url::modify("table/".get('table')."/deletestate/1/page/1")}'"  class="btn btn-primary" />
</div>
...
<th class="s_out"><input title="点击可全选本页的所有项目"  onclick="CheckAll(this.form)" type="checkbox" name="chkall" class="checkbox" /> </th>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="manage" align="center">
                    <a href='<?php if($d['linkto']){echo $d['linkto'];}elseif(front::get('site')!='default'){echo config::get('site_url').'index.php?case=archive&act=show&aid='.$d[$primary_key];}else{echo url("archive/show/aid/$d[$primary_key]",false);}?>' target="_blank" title="查看"><i class="glyphicon glyphicon-eye-open"></i></a>
                    <?php if(chkpower('archive_check') || $d['checked']==0){?><a href="<?php echo modify("act/edit/table/$table/id/$d[$primary_key]/deletestate/".front::get('deletestate'));?>" title="编辑"><i class="glyphicon glyphicon-edit"></i></a><?php } ?>
                </td>
            </tr>
            {/loop}
    ...
    <input  class="btn btn-secondary" type="button" value=" 排序 " name="order" onclick="this.form.action='<?php echo modify('act/batch',true);?>'; this.form.batch.value='listorder'; this.form.submit();"/>

    <?php  if(!front::get('deletestate') && chkpower('archive_check')) {?>
    <input type="button" value=" 审核 " name="check" onclick="if(getSelect(this.form)  && confirm('确实要审核ID为('+getSelect(this.form)+')的记录吗?')){ this.form.action='<?php echo modify('act/batch',true);?>';this.form.batch.value='check';this.form.submit();}" class="btn btn-primary" />
        <?php } ?>

    <?php if(chkpower('archive_del')){ ?>
    <?php if(!front::get('deletestate')) {?>
    <input type="button" value="删除" name="delete" onclick="if(getSelect(this.form) && confirm('确实要把ID为('+getSelect(this.form)+')的记录放到回收站吗?')){this.form.action='<?php echo modify('act/batch',true);?>'; this.form.batch.value='deletestate'; this.form.submit();}" class="btn btn-success"/>


        <?php } ?>

    <input type="button" value="<?php if(get('table')=='archive') {?>彻底<?php } ?>删除" name="delete" onclick="if(getSelect(this.form) && confirm('确实要删除ID为('+getSelect(this.form)+')的记录吗?')){this.form.action='<?php echo modify('act/batch',true);?>'; this.form.batch.value='delete'; this.form.submit();}" class="btn btn-info" />

    <?php if(get('table')=='archive') {?>
    <input type="button" value=" 还原 " name="restore" onclick="if(getSelect(this.form) && confirm('确实要把ID为('+getSelect(this.form)+')的已删除记录还原吗?')){this.form.action='<?php echo modify('act/batch',true);?>'; this.form.batch.value='restore'; this.form.submit();}" class="btn btn-warning" />
        <?php } ?>
   <?php } ?>
      <div class="blank10"></div>
...
<div class="linebreak"><div class="blank10"></div></div><?php } ?>
       移动内容到：<?php echo form::select('catid',0,category::option()); ?>
       <input  class="btn btn-danger" type="button" value="移动" name="movelist" onclick="if(getSelect(this.form)  && confirm('确实要移动ID为('+getSelect(this.form)+')的记录吗?')){ this.form.action='<?php echo modify('act/batch',true);?>';this.form.batch.value='movelist';this.form.submit();}"/>

...
<td class="htmldir" align="center">{if $d['state']}<i class="glyphicon glyphicon-ok"></i>{else}<i class="glyphicon glyphicon-remove"></i>{/if}</td>
...
<input class="btn btn-success" type="button" value="取消审核" name="douncheck" onclick="if(getSelect(this.form) && confirm('确实要取消审核ID为('+getSelect(this.form)+')的记录吗?')){this.form.action='<?php echo modify('act/batch',true);?>'; this.form.batch.value='douncheck'; this.form.submit();}"/>

<input  class="btn btn-info" type="button" value=" 清空 " name="clearall" onclick="if(confirm('确实要清空吗?')){this.form.action='<?php echo modify("/act/clearall/table/$table/token/$token");?>'; this.form.batch.value='clearall'; this.form.submit();}"/>

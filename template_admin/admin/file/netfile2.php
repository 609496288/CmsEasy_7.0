...
<script type="text/javascript" language="JavaScript">
	...
	$(window).load(function(){
		...
		$('#submitbotton').click(function() {
            ...
            if(!/^http(s*):\/\//i.test($('#upfilepath').val())){
                alert('必须以http://或https://开头！');
                $('#upfilepath').focus();
                return false;
            }
            refile($('#upfilepath').val(),1,$('#autoplay').attr('checked'),$('#width').val(),$('#height').val());
			return false;
		});
	});
...
<body class="bodyflow">
<form name="upfile" id="upfile" method="post" action="index.php?case=file&act=netfilesave2&admin_dir={get('admin_dir')}">
		...
		<div class="maneditcontent">
			<table class="formtablewin">
				<tr class="trstyle2">
					...
					<td class="trtitle02"><input id="upfilepath" type="text" name="upfilepath" maxlength="200" size="30" class="infoInput">
				    必须以http://或https://开头</td>
				</tr>
                 <tr class="trstyle2">
				  <td class="trtitle01"></td>
					<td class="trtitle02" id="upfilepath">
                        自动播放：<input type="checkbox" name="autoplay" id="autoplay" value="1" />
				    宽
				    <input name="width" type="text" id="width" size="5" />
				    高
				    <input name="height" type="text" id="height" size="5" /></td>
				</tr>
			</table>

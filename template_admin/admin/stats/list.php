...
<input  class="btn btn-danger" type="button" value=" 删除 " name="delete" onclick="if(getSelect(this.form) && confirm('确实要删除ID为('+getSelect(this.form)+')的记录吗?')){this.form.action='<?php echo modify('act/batch',true);?>'; this.form.batch.value='delete'; this.form.submit();}"/>

<input  class="btn btn-warning" type="button" value=" 清空 " name="clear" onclick="if(confirm('确实要清空记录吗?')){this.form.action='<?php echo modify('act/clear',true);?>'; this.form.batch.value='clear'; this.form.submit();}"/>

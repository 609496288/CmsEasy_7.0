...
function getTitle($archive, $category, $catid, $type)
{
    if (!empty($archive['mtitle'])) {
        ...
        echo $category[$catid]['meta_title'];
        return;
    } else {
...
function listPageJs($total_page, $num, $current)
{//依次传入，总数，每页显示条目，显示链接总数,当前所在页
    $link = array();
    $link_str = '';
    if ($total_page == 0) {
        return '';
...
function listPage($total_page, $num, $current)
{//依次传入，总数，每页显示条目，显示链接总数,当前所在页
    $link = array();
    $link_str = '';
